...
import { editAccountLoader } from './account-edit.js'

const errorLiterals = resolveErrorLiterals('en_US', accountErrorsI18n)

describe('editAccountLoader', () => {
    ...
    it('returns account and literals on success', async () => {
        ...
        const result = await editAccountLoader({
            id: '1',
        ...
        expect(result.literals.title).toBe('Edit Account')
        expect(result.literals.submitButton).toBe('Save Changes')
    })
    ...
    it('uses pt_BR locale for literals', async () => {
        ...
        expect(result.literals.title).toBe('Editar Conta')
        expect(result.literals.submitButton).toBe('Salvar Alterações')
